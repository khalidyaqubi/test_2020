...
@section('navigation3','تعديل مشروع')
@section('navigation1_link','/admin/home')
@section('navigation2_link','/admin/projects')
@section('navigation3_link','/admin/projects/'.$item->id.'/edit')
@section('content')
            ...
            <div class="kt-portlet__head">
                <div class="kt-portlet__head-label">
                    ...
                    <h3 class="kt-portlet__head-title">
                        تعديل مشروع {{$item->title_ar}}
                    </h3>
                </div>
            </div>
            <form class="kt-portlet__body"
                  enctype="multipart/form-data"
                  method="post" action="/admin/projects/{{$item->id}}">
            @csrf
            @method('put')
            <!-- Start Row -->
                <div class="row">
                                ...
                                <select required class="form-control kt-select2 select2-multi" name="p_categories_ids[]"
                                    ...
                                    @foreach($p_categories as $p_category)
                                        <option value="{{$p_category->id}}"
                                                @if(collect($item->p_categories->pluck('id')->toArray())->contains($p_category->id))selected @endif>{{$p_category->name_ar}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4">
                        <div class="form-group ">
                            ...
                            <select class="form-control" name="super_donation">
                                ...
                                <option value="1" @if($item->need_amount>=1)selected @endif>نعم</option>
                                <option value="0" @if($item->need_amount<1)selected @endif>لا</option>
                            </select>
                        </div>
                    </div>
                    <!-- End col -->
                    ...
                    <div class="col-md-12 ">
                        <div class="form-group row">
                            <label class="col-form-label col-lg-12">
                                الصورة الرئيسية
                                <span style="color:red">(550X550)</span>
                            </label>
                            <div class="">
